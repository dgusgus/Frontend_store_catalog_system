...
// ── Validación ─────────────────────────────────────────────
// Actualizar canSubmit para incluir el teléfono:
...
async function handleConfirm() {
  ...
  try {
    // 1. Crear la orden — el backend usa el name/phone del usuario autenticado
    ...
    // 2. Limpiar carrito DESPUÉS de guardar la orden en createdOrder
    cart.clearCart()

    toast.success(`Pedido ${order.orderNumber} creado`)

    // 3. Abrir WhatsApp usando los datos de la orden (no del carrito)
    ...
    <main class="max-w-lg mx-auto px-4 pb-10 pt-4 flex flex-col gap-4">
      ...
      <template v-if="createdOrder">
        ...
        <div class="flex flex-col gap-2">
          ...
          <button class="btn btn-ghost w-full text-base-content/50" @click="router.push('/catalog')">
            Seguir comprando
          </button>
        </div>
      </template>
      ...
      <template v-else>
        ...
        <!-- Botón principal — condicional según si está logueado -->
        <div class="flex flex-col gap-2">

          <!-- NO autenticado → ir a registrarse -->
          <template v-if="!auth.isAuthenticated">
            ...
            <p class="text-center text-sm text-base-content/50">
              ¿No tenés cuenta?
              <RouterLink to="/register" class="link link-primary font-medium">Registrate gratis</RouterLink>
            </p>
          </template>

          <!-- SÍ autenticado → confirmar pedido -->
          <template v-else>
           <!-- Agregar este campo antes del botón confirmar -->
            <div class="card bg-base-100 shadow-sm">
              <div class="card-body p-4 gap-3">
                <h2 class="font-semibold text-sm text-base-content/60 uppercase tracking-wide">
                  Tu teléfono
                ...
                <label class="form-control">
                  <input v-model="customerPhone" type="tel" inputmode="tel" placeholder="71234567"
